    ...
    public abstract class TMethod : ITypeMember
    {
        ...
        public virtual void Resolve(TweedleSystem inSystem, TType inOwnerType)
        {
            ...
            if (!bIsInterop && !ReturnType.Get().IsValidReturnType())
                throw new TweedleLinkerException("Return type " + ReturnType.Name + " is not a valid return type");

            for (int i = 0; i < RequiredParams.Length; ++i)
            {
                RequiredParams[i].Type.Resolve(inSystem);
                if (!bIsInterop && !RequiredParams[i].Type.Get().IsValidIdentifier())
                    throw new TweedleLinkerException("Parameter type " + RequiredParams[i].Type.Name + " is not a valid identifier type");
            }
            for (int i = 0; i < OptionalParams.Length; ++i)
            {
                ...
                if (!bIsInterop && !OptionalParams[i].Type.Get().IsValidIdentifier())
                    throw new TweedleLinkerException("Parameter type " + OptionalParams[i].Type.Name + " is not a valid identifier type");
            }
        }
